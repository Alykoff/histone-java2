 ...
 * Tokenizer is based on regular expressions and returns tokens one by one. All tokens are configured in {@link Tokenizer}
 *
...
public class Tokenizer {
    ...
    public void readTokenToBuffer() {
        ...
        if (lastIndex >= checkIndex) {
            ...
            buffer.add(new Token(Tokens.T_EOF.getId(), checkIndex));
            return;
        }

        if (matcher.find(lastIndex)) {
            if (matcher.start() > lastIndex) {
                buffer.add(new Token(input.substring(lastIndex, matcher.start()), Tokens.T_ERROR.getId(), lastIndex));
            }
            ...
            buffer.add(new Token(input.substring(lastIndex), Tokens.T_ERROR.getId(), lastIndex));
