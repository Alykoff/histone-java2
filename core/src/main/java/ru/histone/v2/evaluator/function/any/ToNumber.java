...
public class ToNumber extends AbstractFunction {
    ...
    public ToNumber(Converter converter) {
        super(converter);
    }
    ...
    public CompletableFuture<EvalNode> execute(Context context, List<EvalNode> args) throws FunctionExecutionException {
        ...
        if (converter.isNumberNode(node)) {
            return getFromNumberNode(node);
        } else if (node.getType() == HistoneType.T_STRING && converter.isNumeric((StringEvalNode) node)) {
            ...
            LocalDateTime dateTime = DateUtils.createDate(converter, dateNode.getValue());
    ...
    private CompletableFuture<EvalNode> getFromNumberNode(EvalNode node) {
        if (node instanceof DoubleEvalNode) {
            ...
            return converter.getNumberFuture(value);
        }
